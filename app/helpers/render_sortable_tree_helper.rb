  ...
  module Render 
    class << self
      ...
      def show_link
        ...
        "<h4>#{ h.link_to(node.send(title_field), url.gsub('%2F', '/')) }</h4>"
      end
      ...
      def controls
        ...
        "
          <div class='controls'>
            #{ h.link_to '', edit_path.gsub('%2F', '/'), :class => :edit }
            #{ h.link_to '', destroy_path.gsub('%2F', '/'), :class => :delete, :method => :delete, :data => { :confirm => 'Are you sure?' } }
          </div>
        "
      end
